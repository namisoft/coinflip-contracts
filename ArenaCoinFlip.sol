...
// we migrate the contract to new one but still want to keeping
...
contract Tracker is ITracker, Ownable {
    ...
    // Events
    event TargetAdded(address indexed target, uint256 time);
    event TargetRemoved(address indexed target, uint256 time);

    function addToTracking(address _target) external override onlyOwner {
        canTrack[_target] = true;
        emit TargetAdded(_target, block.timestamp);
    }

    function removeFromTracking(address _target) external override onlyOwner {
        canTrack[_target] = false;
        emit TargetRemoved(_target, block.timestamp);
    }
}
...
contract GameMaster is IGameMaster, PenguinDef, Withdrawable, Ownable {
    ...
    // Events
    event RegisterGameHouse(
        address indexed gameHouse,
        address indexed owner,
        address factory,
        address paymentCurrency,
        uint256 initialFund,
        address defaultFeeAllocator,
        uint256 time
    );
    event UnregisterGameHouse(
        address indexed gameHouse,
        address withdrawFundTo,
        address indexed caller,
        uint256 time
    );
    event MigrateGameHouse(
        address fromGameHouse,
        address newGameHouseFactory,
        address indexed newGameHouse,
        address indexed tracker,
        address caller,
        uint256 time
    );
    event SetDefaultOperationFee(uint256 value, uint256 time);
    event SetDefaultOperationFeeAllocators(address[] targets, uint256[] shares, uint256 time);
    ...
    Withdrawable(new address[](0)) {
        ...
        if (_defaultFeeAllocator != address(0)) {
            ...
            targets[0] = _defaultFeeAllocator;
            uint256[] memory shares = new uint256[](1);
            shares[0] = MAX_BIPS;
            setDefaultOperationFeeAllocators(targets, shares);
        }
    }
    ...
    external returns (address) {
        ...
        if (totalAllocators > 0) {
            for (uint i = 0; i < defaultOperationFeeAllocators.length; i++) {
                ...
                operationFeeAllocators[address(tracker)].push(allocator);
            }
        }
        ...
        emit RegisterGameHouse(
            address(gameHouse),
            msg.sender,
            address(_factory),
            address(_paymentCurrency),
            _initialFund,
            _defaultFeeAllocator,
            block.timestamp);
    ...
    function unregisterGameHouse(address _gameHouse, address _withdrawFundTo) external {
        ...
        if (msg.sender == owner()) {
            ...
            emit UnregisterGameHouse(_gameHouse, _withdrawFundTo, msg.sender, block.timestamp);
        } else {
            ...
            emit UnregisterGameHouse(_gameHouse, address(0), msg.sender, block.timestamp);
        }
    ...
    function migrateGameHouse(address _fromGameHouse, IGameHouseFactory _factory) external returns (address) {
        ...
        emit MigrateGameHouse(_fromGameHouse, address(_factory), address(newGameHouse), address(tracker), msg.sender, block.timestamp);
    ...
    function setDefaultOperationFee(uint256 _value) public onlyOwner {
        ...
        emit SetDefaultOperationFee(_value, block.timestamp);
    }
    ...
    function setDefaultOperationFeeAllocators(address[] memory _targets, uint256[] memory _shares) public onlyOwner {
        ...
        emit SetDefaultOperationFeeAllocators(_targets, _shares, block.timestamp);
    }
    ...
    function setMinRequiredInitialFund(address _paymentCurrency, uint256 _amount) external onlyOwner {
        ...
        emit SetMinRequiredInitialFund(_paymentCurrency, _amount, block.timestamp);
    }
    ...
    function migrate(IGameMaster _newGameMaster) external onlyOwner {
        ...
        emit Migrate(address(_newGameMaster), block.timestamp);
    }
    ...
    function withdraw(address _token, uint256 _amount, address _to) external onlyOwner {
        ...
        emit Withdraw(_token, _amount, _to, block.timestamp);
    }
...
abstract contract GameHouseBase is IGameHouse, PenguinDef, Ownable, AccessControl {
    ...
    // Events
    event DepositFund(uint256 amount, address from, uint256 time);
    event WithdrawFund(uint256 amount, address withdrawTo, uint256 time);
    event SetPlayerStorage(address newPlayerStorage, uint256 time);
    event SetRandomizer(address newRandomizer, uint256 time);
    event AddGameOperator(address indexed toAdd, uint256 time);
    event RemoveGameOperator(address indexed toRemove, uint256 time);
    event Start(address indexed caller, uint256 time);
    event Stop(address indexed caller, uint256 time);
    event SetOperationFee(uint256 value, uint256 time);
    event SetFee(uint256 value, uint256 time);
    event SetFeeAllocators(address[] targets, uint256[] _shares, uint256 time);
    event AddExtension(address indexed ex, uint256 time);
    event RemoveExtension(address indexed ex, uint256 time);
    event TransferGameMaster(address indexed newGameMaster, uint256 time);
    event Migrate(address indexed toGameHouse, uint256 time);
    ...
    // That includes Penguin operation fee and Game House fee
    ...
    function depositFund(uint256 _amount) external override {
        // Transfer fund from caller to this contract
        paymentCurrency.safeTransferFrom(msg.sender, address(this), _amount);
        // Lock fund
        _lockFund(_amount);
        // emit event
        emit DepositFund(_amount, msg.sender, block.timestamp);
    }
    ...
    function setPlayerStorage(IArenaPlayerStorage _newPlayerStorage) external override onlyGameMaster {
        playerStorage = _newPlayerStorage;
        emit SetPlayerStorage(address(_newPlayerStorage), block.timestamp);
    }
    ...
    function setRandomizer(IRandomizer _randomizer) external onlyGameMaster {
        ...
        randomizer = _randomizer;
        emit SetRandomizer(address(_randomizer), block.timestamp);
    }
    ...
    function setOperationFee(uint256 _value) external override onlyGameMaster {
        ...
        emit SetOperationFee(_value, block.timestamp);
    }

    function setFee(uint256 _value) external onlyOwner {
        require(_value + operationFeePerBet <= MAX_BIPS, "Value exceeded");
        feePerBet = _value;
        emit SetFee(_value, block.timestamp);
    }

    // warning: call this method will replace all current allocators
    function setFeeAllocators(address[] memory _targets, uint256[] memory _shares) public onlyOwner {
        uint256 numAllocators = _targets.length;
        require(numAllocators > 0, "At least 1 allocator required");
        require(numAllocators == _shares.length, "Invalid input: arrays length mismatched");
        ...
        emit SetFeeAllocators(_targets, _shares, block.timestamp);
    }
    ...
    function addExtension(IGameExtension _ex) external override onlyGameMaster {
        ...
        emit AddExtension(address(_ex), block.timestamp);
        _ex.onInit();
    }
    ...
    function removeExtension(IGameExtension _ex) external override onlyGameMaster {
        ...
        emit RemoveExtension(address(_ex), block.timestamp);
        _ex.onDeinit();
    }
    ...
    function migrate(IGameHouse _toGameHouse) external override onlyGameMaster {
        ...
        isRunning = false;
        // event
        emit Migrate(address(_toGameHouse), block.timestamp);
    }
    ...
    function transferGameMaster(IGameMaster _newGameMaster) external override onlyGameMaster {
        ...
        emit TransferGameMaster(address(_newGameMaster), block.timestamp);
    }
...
contract CoinFlipGameHouse is GameHouseBase, IRandomnessConsumer {
    ...
    event FulfillRandomness(uint256 indexed betId, uint256 time);
    event FinalizeGame(uint256 indexed betId, uint256 payoutAmt, uint256 totalFee, uint256 time);
    event CancelGame(uint256 indexed betId, uint256 time);
    event SetMaxBetAmount(uint256 value, uint256 time);
    event SetMinBetAmount(uint256 value, uint256 time);
    event SetPassedBlocksForCancelable(uint256 value, uint256 time);
    event SetResolveGameOnRandomnessFulfillment(bool value, uint256 time);
    ...
    function setMaxBetAmount(uint256 _value) public onlyOwner {
        ...
        maxBetAmt = _value;
        emit SetMaxBetAmount(_value, block.timestamp);
    }
    ...
    function setMinBetAmount(uint256 _value) public onlyOwner {
        ...
        minBetAmt = _value;
        emit SetMinBetAmount(_value, block.timestamp);
    }
    ...
    function fulfillRandomness(uint256 _requestId, uint256 _randomNum) external {
        ...
        if (resolveGameOnRandomnessFulfillment) {
            ...
            // --- Emit event
            emit FulfillRandomness(toResolveBetId, block.timestamp);
        }
    }
    ...
    function setResolveGameOnRandomnessFulfillment(bool _value) external onlyGameMaster {
        ...
        emit SetResolveGameOnRandomnessFulfillment(_value, block.timestamp);
    }
    ...
    function _finalizeGame(uint56 _betId) internal {
        ...
        // emit event
        emit FinalizeGame(_betId, payoutAmt, totalFee, block.timestamp);
    ...
    function _cancelGame(uint56 _betId, bool _canceledByPlayer) internal {
        ...
        // emit event
        emit CancelGame(_betId, block.timestamp);
